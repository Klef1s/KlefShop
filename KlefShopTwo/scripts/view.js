...
//аним.фильтрации
let parent_filter = document.querySelector(".filter");
let item_filter = parent_filter.querySelectorAll(".filter__button");

for (var i = 0; i < item_filter.length; i++) {
  item_filter[i].addEventListener("click", function (event) {
    for (var j = 0; j < item_filter.length; j++) {
      item_filter[j].classList.remove("active");
    }
    this.classList.add("active");
  });
}
